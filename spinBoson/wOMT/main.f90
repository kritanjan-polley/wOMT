program main
    ...
    character(len=100) :: nStr, lamStr, tempStr, tauStr, filename, file_end_st, command_arg
    ...
    allocate(qbath(bath, tlen+2))
    allocate(pbath(bath, tlen+2))
    ...
    allocate(pin(bath,lmax), qin(bath,lmax))
    ...
    do j=1,lmax
        do i=1,bath
            pin(i,j) = rand_normal(shorttab1(i))
            qin(i,j) = rand_normal(shorttab2(i))
        end do
    end do
    ...
    print*, "The Hamiltonian has the form ..."
    call print2d(hamil*omegac)
    ! call print2d(sigmazr)
    ...
    outer : do j=1,lmax
        ...
        loop_n_initial : do
            ...
            nin_loop(fixint) = one - sum(nin_loop(2:nsys+1))
            if ((nin_loop(fixint) >= ninitial(fixint) - gamma) .and. &
                (nin_loop(fixint) <= ninitial(fixint) + gamma)) then
                exit loop_n_initial
            end if
        end do loop_n_initial
        ...
        if ( (maxval(sum(pop, dim=1))-one > tol) )  then
            print*, 'bad trajectory', j, ' total population is ', maxval(sum(pop, dim=1))
            m = m + 1
            cycle outer
        end if

        if ( (one-minval(sum(pop, dim=1)) > tol) )  then
            print*, 'bad trajectory', j, ' total population is ', minval(sum(pop, dim=1))
            m = m + 1
            cycle outer
        end if
    ...
    ! filename = "SB_wOMT_"//trim(nStr)//"LS_lambda_"//trim(lamStr)//"_T_"// & 
    !             trim(tempStr)//"_tau_"//trim(tauStr)//".txt"
    ! filename = "SB_model"//trim(nStr)//"_wOMT_"//trim(command_arg)//".txt"
    filename = "SB_model"//trim(nStr)//"_wOMT.txt"
