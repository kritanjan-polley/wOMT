program main
    ...
    integer :: mi, err_main, im, jm, track_bad_trj, int1, int2
    ...
    character(len=100) :: nStr, filename, command_arg
    ...
    call check_hermitian(Hamil_e, "Hamiltonian")
    call print2d(Hamil_e*omegac)
    ...
    do im = 1, num_traj
        ...
        if (mod(im,(num_traj/20)).eq.0) then
            ...
            print*,"Done with n_traj=",im, 'after', finish-start, 'seconds'
            call flush()
        end if
    ...
    filename = "SB_model"//trim(nStr)//"_MASH_"//trim(command_arg)//".txt"
    ! filename = "SB_model"//trim(nStr)//"_MASH.txt"
    ...
    subroutine initial_sampling_bath
        ...
        !! initial bath, Wigner
        do mi=1,nbath
            p(mi) = rand_normal(shorttab1(mi))
            q(mi) = rand_normal(shorttab2(mi))
        end do
