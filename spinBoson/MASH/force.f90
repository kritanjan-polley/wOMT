module force
    ...
    subroutine pot_add_bath(q, V)
        ...
        real(kind=wp1), intent(in) :: q(nbath)
        real(kind=wp1), intent(inout) :: V(nsys, nsys)

        real(kind=wp1) :: V0, V1

        V0 = dot_product(mw2, q*q)*half
        V1 = dot_product(ci,q)
        V = Hamil_e
        V(1, 1) = V(1, 1) + V0 + V1
        V(2, 2) = V(2, 2) + V0 - V1
    ...
    subroutine grad(q, G)
        real(kind=wp1), intent(in) :: q(nbath)
        real(kind=wp1), intent(out) :: G(nbath, nsys, nsys)
    ...
    subroutine grad_adiabatic(q, U, Gad)
        ...
        real(kind=wp1), intent(out) :: Gad(nbath, nsys, nsys)
        real(kind=wp1), allocatable :: Gdia(:,:,:)
        integer :: i, ierror

        allocate(Gdia(nbath, nsys, nsys), stat=ierror)
    ...
    subroutine grad_adiabatic_diagonal(q, U, a, dvdq)
        real(kind=wp1), intent(in) :: q(:), U(:,:)
        integer, intent(in) :: a
        real(kind=wp1), intent(out) :: dvdq(nbath)
        real(kind=wp1), allocatable :: Gdia(:,:,:)
        integer :: ii, ierror
    ...
    subroutine nonAdCoup(q,Vad,U,d)
        ...
        real(kind=wp1), intent(out) :: d(nbath, nsys, nsys)
        integer :: gi, i, j

        real(kind=wp1), allocatable :: Gad(:,:,:)
        allocate(Gad(nbath, nsys, nsys), stat=gi)
    ...
    subroutine nonAdCoupDirection(q, cad, Vad, U, n, m, dj)
        ...
        real(kind=wp1), intent(out) :: dj(nbath)
        integer, intent(in) :: n, m
        integer :: i, gi

        real(kind=wp1), allocatable :: d(:,:,:)
        allocate(d(nbath, nsys, nsys), stat=gi)
