program main
    ...
    !Variable directory
    real(kind=wp1), dimension(:,:), allocatable :: pin, qin, traj_count
    ...
    real(kind=wp1), dimension(:), allocatable :: shorttab1, shorttab2, nin_loop, ninitial, &
                                  count_pop, count_coh, sum_all, sum_nz, coh_count_time
    ...
    nStr = str(model)
    filename = "SB_model"//trim(nStr)//"_SQC.txt"
